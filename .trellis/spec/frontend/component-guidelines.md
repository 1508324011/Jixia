...
- Document-grounded AI copilots should be chat-native workbench panes, not inspector forms. They must use a clear work-surface anatomy: compact header/mode bar, durable conversation viewport, source/artifact region or drawer, stable history access, and sticky command composer. They must present explicit context chips, source/provenance cards, visible server-run state, safe message actions, provider/model status, and no-writeback affordances.
