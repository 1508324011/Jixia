...
### 1. Scope / Trigger
- Trigger: `packages/shared/src/**` contains transport-safe MVP contracts for auth/project membership, documents/editor snapshots, attachments/upload intents, private AI configuration/conversations, and the owner-scoped literature foundation.
...
- Literature module: discriminated personal/project scope DTOs, closed typed assertion inputs and projections, provider-record provenance views, deterministic Literature replay views, and semantic contract DTOs for relations, imports, source revisions, annotations, excerpts, evidence, notebook projections, and citation occurrences.
...
- API remains the authority for permissions and business rules. Shared DTO names may describe request shapes but must not imply that frontend code can decide document, attachment, project, AI, literature, or audit access.
- Locked MVP values must match `doc/MVP_rule.md`: project roles, document types/statuses, supported block types, attachment upload statuses/failure reasons, upload limits, AI config view fields, private AI conversation model, literature assertion/relation kinds, and redaction-sensitive omissions.
- Literature ownership must be a discriminated union. Shared create contracts accept personal scope or a project ID and never accept `ownerUserId`, actor identity, membership state, or a client authorization decision.
- Literature assertion values must use a closed discriminated union for `title`, `abstract`, `publicationYear`, and `doi`. Provider-native records, generic payload JSON, storage references, source bodies, and server projection state are not shared assertion input.
- Literature replay views retain assertion/provider provenance, ordered history, and conflicts. Rendered citations, bibliography strings, provider rankings, merge decisions, and reconciliation state remain outside Phase 1 contracts.
- Route-less literature concepts are semantic DTOs only. Their presence in shared contracts does not authorize browser creation, mutation, persistence access, provider execution, or worker behavior.
...
- Literature request accepts `ownerUserId`, provider payloads, arbitrary assertion JSON, rendered citation strings, or permission flags -> block PR because identity, normalization, authorization, and rendering remain server-owned.
...
- Good: `CreateLiteratureRequest` carries `{ scope: "personal" }` or `{ scope: "project", projectId }`, while the API derives the actor and authorization.
- Good: a Literature projection returns ordered assertion history and current/conflict values with assertion and provider-record IDs, but no provider-native payload.
...
- Bad: a shared Literature helper decides whether a role can append assertions, or a citation DTO stores a rendered number or bibliography string as canonical state.
...
- Literature contract review: prove ownership and assertion inputs are exhaustive discriminated unions, create requests cannot submit personal owner IDs, and replay DTOs preserve provenance without provider payloads.
