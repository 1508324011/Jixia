...
For LAN/manual review, make the API and signed local object-storage URL reachable from the browser and explicitly authorize the web origin. Example: if the browser opens Jixia at reviewer origin `http://10.128.253.195:5173`, use API origin `http://10.128.253.195:3000` and start the API with `ATTACHMENT_STORAGE_DRIVER=local`, `API_HOST=0.0.0.0`, `LOCAL_OBJECT_STORAGE_PUBLIC_BASE_URL=http://10.128.253.195:3000/local-object-storage`, and `LOCAL_OBJECT_STORAGE_ALLOWED_ORIGINS=http://10.128.253.195:5173`. If `LOCAL_OBJECT_STORAGE_ALLOWED_ORIGINS` is omitted in development, Jixia still derives `http://<LOCAL_OBJECT_STORAGE_PUBLIC_BASE_URL host>:5173` as a safe same-host convenience; committed defaults never allow wildcard origins. During review, the browser Network panel should show `POST http://10.128.253.195:3000/api/attachments/upload-intents` -> credentialless `PUT http://10.128.253.195:3000/local-object-storage/upload/...` -> `POST http://10.128.253.195:3000/api/attachments/upload-intents/:id/confirm` -> `POST http://10.128.253.195:3000/api/attachments/:id/download`.
