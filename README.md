...
- `POST /api/projects/:projectId/library/adoptions` is the explicit project-source adoption path; the browser sends only `{ sourceLibraryEntryId }`, while the server checks source readability plus project owner/editor membership before creating or reusing a project-scoped `LibraryEntry`. Successful first-time adoptions create the target `LibraryEntry` and a sanitized `project_library.source_adopted` governance audit row in one transaction; repeated, denied, or rejected attempts do not emit duplicate adoption audits.
