...
That runbook covers the exact reset/start commands, the packaged bundle path generated by `npm run package:native-demo`, the current truthful browser walkthrough, and the boundary between the shipped workbench reset and the still-pending operator/recommendation work.
...
The browser surfaces already reflect the route/shell/ownership reset, including the explicit notebook-to-project projection affordance.
...
2. keep the browser-side notebook-to-project projection flow truthful as the workbench evolves
