...
That runbook covers the exact reset/start commands, the packaged bundle path generated by `npm run package:native-demo`, user-owned runtime paths, and the real click-path through `Create space`, `Open library`, `Import paper`, `Open reader`, `Refresh reader`, `Open writing`, `Reload draft`, `Publish`, and the optional `Run governed summary` finale.
...
API, and the health endpoint at `/health`. The browser walkthrough is now `Create space` ->
`Open library` -> `Import paper` -> `Open reader` -> `Refresh reader` -> `Open writing` ->
`Reload draft` -> `Publish` -> optional `Run governed summary`.
