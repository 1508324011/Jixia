...
**Goal:** Turn the verified Task 10 scholarly web shell into a reproducibly runnable lab-server package with a minimal server startup path, Docker-first scaffolding, and bilingual operator documentation.

**Architecture:** Task 11 should stay narrow. Do not refactor the domain model or start wiring full browser-side live data. Instead, add the thinnest truthful runtime that can start Jixia on Node 22, expose a health endpoint, serve the built Task 10 shell, and document persistent storage/database locations for operators. Docker and compose should encode that runtime, while smoke tests lock the operator contract so Task 12 can focus on migration checkpoints rather than startup confusion.
...
- Branch: `task10-layered-ui`
- Verified branch state before this plan: `npm run test`, `npm run typecheck`, and `npm run build` pass.
- `src/server/index.ts` exports `createJixiaApp`, but there is no real HTTP listener or operator startup command yet.
- `.env.example` already exists, but it only contains placeholder variable lines.
- `README.md` and `README_CN.md` mention Task 11 direction, but they do not yet provide a concrete operator runbook.
- `Dockerfile`, `docker-compose.yml`, `.dockerignore`, and `tests/smoke/deploy-docs.test.ts` do not exist yet.
...
1. Keep the current `task10-layered-ui` visual shell intact. Task 11 is about making it runnable and operable, not redesigning pages.
2. The server runtime must stay aligned with the server-first boundary. Add only the minimal Node HTTP layer needed to expose health and serve the built shell.
...
- a short note that the current runtime serves the Task 10 shell and health endpoint, but does not yet imply full browser-side live data integration
...
4. `README.md` and `README_CN.md` both function as operator runbooks.
